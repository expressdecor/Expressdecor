...
class Mage_Catalog_Model_Resource_Eav_Mysql4_Url extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function saveRewrite($rewriteData, $rewrite)
    {
        if ($rewrite && $rewrite->getId()) {
            if ($rewriteData['request_path'] != $rewrite->getRequestPath()) {
                // we shouldn't rewrite user defined rewrites
                
                if ($rewriteData['target_path'] == $rewrite->getTargetPath()) {
                    $rewriteData['target_path'] = $rewrite->getRequestPath();
                    $this->saveRewriteHistory($rewriteData);
                } else {
            ...
            try {
                ...
                if(!empty($rewriteData['product_id'])) {
                    ...
                    $results = $conn->fetchAll("SELECT count(*) as num FROM core_url_rewrite where product_id=".$rewriteData['product_id']);
    ...
    public function saveRewriteHistory($rewriteData)
    {
        ...
        if ($rewrite === false) {
            ...
            // ED changes
            //this will check if an RP has been already created 
            ///////////////////////////////////////////////////////////////////////
            $tmpRewriteData = $rewriteData->getData();
            if(!empty($tmpRewriteData['category_id'])
                && empty($tmpRewriteData['product_id'])
                && !empty($tmpRewriteData['options'])
                && $tmpRewriteData['options'] == 'RP' ) {
                
                /*    
                $conn = Mage::getSingleton('core/resource')->getConnection('core_read');
                $results = $conn->fetchAll("SELECT count(*) as num FROM core_url_rewrite where category_id=".$tmpRewriteData['category_id']." and options='RP'");
                
                if($results[0]['num'] == 0)
                    $this->_getWriteAdapter()->insert($this->getMainTable(), $rewriteData->getData());
                */
            }
            else if(!empty($tmpRewriteData['product_id'])) {
                $conn = Mage::getSingleton('core/resource')->getConnection('core_read');
                $results = $conn->fetchAll("SELECT count(*) as num FROM core_url_rewrite where product_id=".$tmpRewriteData['product_id']);
                ...
                try {
                    ...
                    Mage::cDebug('Eav/Mysql4/Url.php', $this->getMainTable() . ' ' . $rewriteData->getRequestPath()." ". print_r($rewriteData->getData(),true). " " . $e->getMessage());
                }
            }
            ///////////////////////////////////////////////////////////////////////
        }
