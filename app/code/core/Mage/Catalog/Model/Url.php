...
class Mage_Catalog_Model_Url
{
    ...
    protected function _refreshCategoryRewrites(Varien_Object $category, $parentPath = null, $refreshProducts = true)
    {
        if ($category->getId() != $this->getStores($category->getStoreId())->getRootCategoryId()) {
            ...
            ////////////////
            if ($this->_rewrite && $rewriteData['target_path'] == $this->_rewrite->getTargetPath() 
                && $rewriteData['request_path'] != $this->_rewrite->getRequestPath()) {
                $beforeSetRewriteHistory = $this->getShouldSaveRewritesHistory($category->getStoreId());
                $this->setShouldSaveRewritesHistory(false);
            ...
            //////////////////
            if (isset($beforeSetRewriteHistory)) {
                $this->setShouldSaveRewritesHistory($beforeSetRewriteHistory);
            }
            //////////////////
