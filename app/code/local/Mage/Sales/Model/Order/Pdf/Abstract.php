...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
	...
	protected function insertLogo(&$page, $store = null)
	{
		...
		if ($image) {
			...
			if (is_file($image)) {
				...
				$page->drawImage($image, 15, 800, 125, 825); //Alex changes 7/20
			}
		}
		//return $page;
	}
	...
	protected function insertAddress(&$page, $store = null)
	{
		...
		$this->_setFontRegular($page, 10); //Alex changes 7/20
	...
	protected function insertOrder(&$page, $obj, $putOrderId = true, $type_document='any')
	{
		...
		$page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92)); //Alex changes 7/20 lines 194-198
		...
		$page->drawText(Mage::helper('sales')->__('Order Date: ') . Mage::helper('core')->formatDate($order->getCreatedAtStoreDate(), 'medium', false), 25, 760, 'UTF-8');

		$page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
		$page->setLineColor(new Zend_Pdf_Color_GrayScale(0.5));
		$page->setLineWidth(0.5);
		$page->drawRectangle(15, 755, 275, 730); //Alex changes 7/20
		$page->drawRectangle(275, 755, 580, 730); //Alex changes 7/20
		...
		if (!$order->getIsVirtual()) {
			$page->drawText(Mage::helper('sales')->__('SHIP TO:'), 285, 740 , 'UTF-8'); //Alex changes 7/20
		}
		else {
			if ($type_document !== 'packingslip' ) //Alex changes 7/21
		...
		$page->drawRectangle(15, 730, 580, $y-10);//Alex changes 7/20
		$page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
		$this->_setFontRegular($page,10);//Alex changes 7/20
		$this->y = 720;
		//Alex changes 7/20 begin
		foreach ($billingAddress as $value){
			if ($value!=='') {
				$this->_setFontRegular($page,12);
				if (strpos(ltrim($value),',') !== false) {
					$page->drawText(substr(strip_tags(ltrim($value)),0,strpos(strip_tags(ltrim($value)),',')+1), 20, $this->y, 'UTF-8');
					$this->y -=10;
					$page->drawText(substr(strip_tags(ltrim($value)),strpos(strip_tags(ltrim($value)),',')+2,strlen(strip_tags(ltrim($value)))-strpos(strip_tags(ltrim($value)),',')+2), 20, $this->y, 'UTF-8');  //Alex changes 7/20
				} else {
					$page->drawText(strip_tags(ltrim($value)), 20, $this->y, 'UTF-8');
				}
		...
		if (!$order->getIsVirtual()) {
			...
			foreach ($shippingAddress as $value){
				if ($value!=='') {
					...
					if (strpos(ltrim($value),',') !== false) {
						$page->drawText(substr(strip_tags(ltrim($value)),0,strpos(strip_tags(ltrim($value)),',')+1), 295, $this->y, 'UTF-8');
						$this->y -=10;
						$page->drawText(substr(strip_tags(ltrim($value)),strpos(strip_tags(ltrim($value)),',')+2,strlen(strip_tags(ltrim($value)))-strpos(strip_tags(ltrim($value)),',')+2), 295, $this->y, 'UTF-8');  //Alex changes 7/20
					} else {
						$page->drawText(strip_tags(ltrim($value)), 295, $this->y, 'UTF-8');  //Alex changes 7/20
					}
			...
			//Alex changes 7/20 end
			$page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
			$page->setLineWidth(0.5);
			if ($type_document !== 'packingslip' ) {//Alex changes 7/21
				$page->drawRectangle(15, $this->y, 275, $this->y-25); //Alex changes 7/20
				$page->drawRectangle(275, $this->y, 580, $this->y-25);//Alex changes 7/20
			} else {
				$page->drawRectangle(15, $this->y, 580, $this->y-25); //Alex changes 7/20
			}
			$this->y -=15;
			$this->_setFontBold($page,10); //Alex changes 7/20
			$page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
			if ($type_document !== 'packingslip' ) {//Alex changes 7/21
		...
		if (!$order->getIsVirtual()) {
			...
			if ($type_document !== 'packingslip' ) {//Alex changes 7/21
			$page->drawText($totalShippingChargesText, 285, $yShipments-7, 'UTF-8'); //Alex changes 7/21
			} else {
			$page->drawText($totalShippingChargesText, 35, $yShipments-7, 'UTF-8');	
			}
			$yShipments -=13;  //Alex changes 7/21
			...
			if (count($tracks)) {
				$page->setFillColor(new Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
				$page->setLineWidth(0.5);
				if ($type_document !== 'packingslip' ) {//Alex changes 7/21 begin
				$page->drawRectangle(285, $yShipments, 525, $yShipments - 10);
				$page->drawLine(380, $yShipments, 380, $yShipments - 10);
				//$page->drawLine(510, $yShipments, 510, $yShipments - 10);
				} else {
				$page->drawRectangle(35, $yShipments, 260, $yShipments - 10);
				$page->drawLine(140, $yShipments, 140, $yShipments - 10);	
				}
				$this->_setFontRegular($page);
				$page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
				...
				$page->drawText(Mage::helper('sales')->__('Title'), 40, $yShipments - 7, 'UTF-8');
				$page->drawText(Mage::helper('sales')->__('Number'), 145, $yShipments - 7, 'UTF-8');					
				} //alex changes end
				$yShipments -=19; // end of alex changes 7/21
				$this->_setFontRegular($page, 10);  //Alex changes 7/21
				foreach ($tracks as $track) {
					...
					$page->drawText($truncatedTitle, 290, $yShipments , 'UTF-8');  //Alex changes 7/21
					$page->drawText($track->getNumber(), 385, $yShipments , 'UTF-8');  //Alex changes 7/21
					} else {
					$page->drawText($truncatedTitle, 40, $yShipments , 'UTF-8');  //Alex changes 7/21
					$page->drawText($track->getNumber(), 145, $yShipments , 'UTF-8');  //Alex changes 7/21						
					} // end of alex changes 7/21
					$yShipments -=8; // end of alex changes 7/21
				}
			} else {
				$yShipments -= 7;
			}
			...
			$page->drawLine(15, $this->y + 15, 15, $currentY); //Alex changes 7/20 lines 372-374
			$page->drawLine(15, $currentY, 580, $currentY);
			$page->drawLine(580, $currentY, 580, $this->y + 15);
	...
	protected function insertTotals($page, $source){
		...
		foreach ($totals as $total) {
			...
			->setSource($source);
			//Alex changes 7/20 lines 447-459
			if ($total->canDisplay()) {
	...
	protected function _setFontRegular($object, $size = 10)//Alex changes 7/20
	{
	...
	protected function _setFontBold($object, $size = 10) //Alex changes 7/20
	{
	...
	protected function _setFontItalic($object, $size = 10) //Alex changes 7/20
	{
	...
	public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array())
	{
		foreach ($draw as $itemsProp) {
			...
			foreach ($lines as $line) {
				$maxHeight = 0;
				foreach ($line as $column) {
					$fontSize = empty($column['font_size']) ? 10 : $column['font_size']; //Alex changes 7/20
