...
class Unirgy_RapidFlow_Model_Profile extends Mage_Core_Model_Abstract
{
    ...
    */
    ...
    protected function _ftpDownload()
    {
        ...
        @unlink($localFile);
        $result = @ftp_get($conn, $localFile, $this->getFilename(), FTP_BINARY);
        if (!$result) {
            $e = error_get_last();
    ...
    protected function _ftpUpload()
    {
        ...
        $conn = $this->_ftpOpen();
        $localFile = $this->getIo()->getFilepath($this->getFilename());
        $result = @ftp_put($conn, $this->getFilename(), $localFile, FTP_BINARY);
        if (!$result) {
            $e = error_get_last();
    ...
    protected function _getExcelRow($r, $l=array())
    {
        ...
        foreach ($r as $k=>$v) {
            ...
            if ($v!=='') {
                ...
                $out .= '<Data ss:Type="String">'.htmlspecialchars($v).'</Data></Cell>';
            } elseif ($type) {
                $out .= '</Cell>';
            }
        }
