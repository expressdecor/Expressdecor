    ...
    public function export()
    {
        ...
        if ($columns) {
            ...
            foreach ($this->_attributesByCode as $k=>$a) {
                ...
                $this->_fields[$k] = array('field' => $k, 'title' => $k, 'column_num' => $i++);
    ...
    protected function _importValidateNewData()
    {
        ...
        foreach ($this->_newData as $sku=>$p) {
            try {
                ...
                $dynPrice = ($typeId=='configurable' || $typeId=='bundle') && (isset($p['price_type']) && !empty($p['price_type']) || !isset($p['price_type']) && !empty($oldProduct['price_type']));
                $dynWeight = ($typeId=='configurable' || $typeId=='bundle') && (isset($p['weight_type']) && !empty($p['weight_type']) || !isset($p['weight_type']) && !empty($oldProduct['weight_type']));
            ...
            try {
                ...
                if ($isNew) {
                    ...
                    if ($hasRequiredOptions) {
                        $this->_insertEntity[$sku]['required_options'] = $hasOptions ? 1 : 0;
                    }
                    $pId = null;
                } else {
                    $pId = $this->_skus[$sku];
                        ...
                        if ($insert || $delete) {
                            ...
                            if ($hasCategoryIds) {
                                if ($isNew) {
                                    ...
                                    $newValue = $deleteOldCat ? $newValue : array_unique(array_merge($oldValue, $newValue));
