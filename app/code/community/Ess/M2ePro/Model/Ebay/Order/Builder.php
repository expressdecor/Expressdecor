...
class Ess_M2ePro_Model_Ebay_Order_Builder extends Mage_Core_Model_Abstract
{
    ...
    private function processOrdersContainingItemsFromCurrentOrder()
    {
        ...
        foreach ($relatedOrders as $order) {
            if ($order->canCancelMagentoOrder()) {
                ...
                Mage::getModel('M2ePro/Order_Log')->add(
    ...
    private function processMagentoOrderUpdates()
    {
        ...
        if ($this->hasUpdate(self::UPDATE_COMPLETED_CHECKOUT)
            || $this->hasUpdate(self::UPDATE_COMPLETED_PAYMENT)
            || $this->hasUpdate(self::UPDATE_COMPLETED_SHIPPING)
        ) {
            $magentoOrderUpdater->updateStatus($this->order->getChildObject()->getStatusForMagentoOrder());
        }

        if ($this->hasUpdate(self::UPDATE_PAYMENT_DATA)) {
            $magentoOrderUpdater->updatePaymentData($this->order->getProxy()->getPaymentData());
        }

        if ($this->hasUpdate(self::UPDATE_COMPLETED_CHECKOUT)) {
            $magentoOrderUpdater->updateShippingAddress($this->order->getProxy()->getAddressData());
            $magentoOrderUpdater->updateCustomerAddress($this->order->getProxy()->getAddressData());
        }

        if ($this->hasUpdate(self::UPDATE_CHECKOUT_MESSAGE)) {
            $magentoOrderUpdater->updateComments($this->order->getProxy()->getChannelComments());
        }
