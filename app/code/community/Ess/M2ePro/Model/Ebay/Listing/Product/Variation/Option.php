...
class Ess_M2ePro_Model_Ebay_Listing_Product_Variation_Option extends Ess_M2ePro_Model_Component_Child_Ebay_Abstract
{
    ...
    protected function getConfigurablePriceParent()
    {
        ...
        foreach ($configurableAttributes as $tempAttribute) {
            ...
            $label = isset($tempAttribute['label']) ? strtolower($tempAttribute['label']) : '';
            $frontLabel = isset($tempAttribute['frontend_label']) ? strtolower($tempAttribute['frontend_label']) : '';
            $storeLabel = isset($tempAttribute['store_label']) ? strtolower($tempAttribute['store_label']) : '';
            ...
            foreach ($tempAttribute['values'] as $tempOption) {
                ...
                $label = isset($tempOption['label']) ? strtolower($tempOption['label']) : '';
                $defaultLabel = isset($tempOption['default_label']) ? strtolower($tempOption['default_label']) : '';
                $storeLabel = isset($tempOption['store_label']) ? strtolower($tempOption['store_label']) : '';
