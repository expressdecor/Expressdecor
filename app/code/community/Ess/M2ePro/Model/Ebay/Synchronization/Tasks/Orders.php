...
class Ess_M2ePro_Model_Ebay_Synchronization_Tasks_Orders extends Ess_M2ePro_Model_Ebay_Synchronization_Tasks
{
    ...
    public function process()
    {
        ...
        //-----------------------------
        $receiveMode = (bool)(int)$config->getGroupValue($this->configGroup.'receive/', 'mode');
        $cancellationMode = (bool)(int)$config->getGroupValue($this->configGroup.'cancellation/', 'mode');

        if (!$receiveMode && !$cancellationMode) {
            return false;
        }
        //-----------------------------
