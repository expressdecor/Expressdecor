    ...
    extends Ess_M2ePro_Model_Ebay_Synchronization_Tasks
{
    ...
    private function getChangedListingsProductsForAccount($account, $sinceTime)
    {
        ...
        // ->__('Task "Update Listings Products" for eBay account: "%acc%" is started. Please wait...')
        $status = 'Task "Update Listings Products" for eBay account: "%acc%" is started. Please wait...';
        $tempString = str_replace('%acc%',$account['title'],Mage::helper('M2ePro')->__($status));
        ...
        $status = <<<STATUS
Task "Update Listings Products" for eBay account: "%acc%" is in data processing state. Please wait...
STATUS;
        $tempString = str_replace('%acc%',$account['title'],Mage::helper('M2ePro')->__($status));
        $this->_lockItem->setStatus($tempString);
                                                    ...
                                                    $ebayChange)
    {
        ...
        if ($tempEbayChanges['status'] != $tempListingProductModel->getStatus()) {
            ...
            Mage::getModel('M2ePro/ProductChange')->updateAttribute(
                $tempListingProductModel->getProductId(),
                'ebay_listing_product_status',
