...
class Ess_M2ePro_Model_Amazon_Order_Item extends Ess_M2ePro_Model_Component_Child_Amazon_Abstract
{
    ...
    {
        /** @var $amazonAccount Ess_M2ePro_Model_Amazon_Account */
        $amazonAccount = $this->getParentObject()->getOrder()->getAccount()->getChildObject();
        $amazonItem = $this->getAmazonItem();

        if (!is_null($amazonItem) && !$amazonAccount->isMagentoOrdersListingsModeEnabled()) {
            throw new Exception(
                'Magento Order creation for items listed by M2E Pro is disabled in Account settings.'
            );
        }

        if (is_null($amazonItem) && !$amazonAccount->isMagentoOrdersListingsOtherModeEnabled()) {
            throw new Exception(
