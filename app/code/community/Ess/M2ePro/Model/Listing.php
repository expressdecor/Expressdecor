...
class Ess_M2ePro_Model_Listing extends Ess_M2ePro_Model_Component_Parent_Abstract
{
    ...
    public function getGeneralTemplate()
    {
        if (is_null($this->generalTemplateModel)) {
            $this->generalTemplateModel = Mage::helper('M2ePro/Component')->getComponentObject(
                $this->getComponentMode(),'Template_General',$this->getData('template_general_id')
            );
        }
    ...
    public function getSellingFormatTemplate()
    {
        if (is_null($this->sellingFormatTemplateModel)) {
            $this->sellingFormatTemplateModel = Mage::helper('M2ePro/Component')->getComponentObject(
                $this->getComponentMode(),'Template_SellingFormat',$this->getData('template_selling_format_id')
            );
        }
    ...
    public function getDescriptionTemplate()
    {
        if (is_null($this->descriptionTemplateModel)) {
            $this->descriptionTemplateModel = Mage::helper('M2ePro/Component')->getComponentObject(
                $this->getComponentMode(),'Template_Description',$this->getData('template_description_id')
            );
        }
    ...
    public function getSynchronizationTemplate()
    {
        if (is_null($this->synchronizationTemplateModel)) {
            $this->synchronizationTemplateModel = Mage::helper('M2ePro/Component')->getComponentObject(
                $this->getComponentMode(),'Template_Synchronization',$this->getData('template_synchronization_id')
            );
        }
    ...
    public static function removeDeletedProduct($product)
    {
        ...
        foreach ($variationOptions as $variationOption) {
            ...
            $listingsAreProcessed[] = array(
                'listing_id' => $listingProduct->getListingId(),
        ...
        $listingsAreProcessed = array_unique($listingsAreProcessed);

        foreach ($listingsAreProcessed as $listing) {

            Mage::getModel('M2ePro/Listing_Log')
                ->setComponentMode($listing['component_mode'])
                ->addProductMessage($listing['listing_id'],
                                    $productId,
