...
class Ess_M2ePro_Block_Adminhtml_Listing_Search_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    // ####################################
        ...
        $this->addColumn('status',
            array(
                ...
                'options' => array(
                    ...
                    Ess_M2ePro_Model_Listing_Product::STATUS_BLOCKED    => Mage::helper('M2ePro')->__('Blocked')
                ),
    ...
    public function callbackColumnProductTitle($value, $row, $column, $isExport)
    {
        ...
        $urlParams['back'] = Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_listing/search');

        $listingUrl = $this->getUrl('*/adminhtml_'.$row->getData('component_mode').'_listing/view',$urlParams);
    ...
    public function callbackColumnActions($value, $row, $column, $isExport)
    {
        ...
        $filter = 'product_id[from]='.$row->getData('product_id').'&product_id[to]='.$row->getData('product_id');
        $link = $this->getUrl('*/adminhtml_'.$row->getData('component_mode').'_listing/view/',array(
            'id'=>$row->getData('listing_id'),
            'filter'=>base64_encode($filter)
        ));

        $html = '<div style="float:right; margin:5px 15px 0 0;">';
        $html .= '<a alt="'.$altTitle.'" title="'.$altTitle.'" target="_blank" href="'.$link.'">';
        $html .= '<img src="'.$iconSrc.'" /></a>';
        $html .= '</div>';
    ...
    {
        return $this->getUrl('*/adminhtml_listing/searchGrid', array('_current'=>true));
    }
