...
class Ess_M2ePro_Block_Adminhtml_Amazon_Listing_Search_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    // ####################################
...
//                             '(csi.product_id = `main_table`.product_id)',array('qty'))
    ...
    public function callbackColumnProductTitle($value, $row, $column, $isExport)
    {
        ...
        $urlParams['back'] = Mage::helper('M2ePro')->makeBackUrlParam('*/adminhtml_amazon_listing/search');

        $listingUrl = $this->getUrl('*/adminhtml_amazon_listing/view',$urlParams);
    ...
    public function callbackColumnActions($value, $row, $column, $isExport)
    {
        ...
        $link = $this->getUrl('*/adminhtml_amazon_listing/view/',array('id'=>$row->getData('listing_id'),
                              'filter'=>base64_encode('product_id[from]='
                                                      .$row->getData('product_id')
                                                      .'&product_id[to]='
                                                      .$row->getData('product_id'))));
