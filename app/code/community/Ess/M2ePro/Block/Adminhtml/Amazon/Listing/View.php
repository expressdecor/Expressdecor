...
class Ess_M2ePro_Block_Adminhtml_Amazon_Listing_View extends Mage_Adminhtml_Block_Widget_Grid_Container
{
    ...
    public function getGridHtml()
    {
        ...
        $taskCompletedSuccessMessage = $helper->escapeJs(
            $helper->__('"%title%" task has successfully submitted to be processed.')
        );
        $taskCompletedWarningMessage = $helper->escapeJs(
            $helper->__('"%title%" task has completed with warnings. <a href="%url%">View log</a> for details.')
        );
        $taskCompletedErrorMessage = $helper->escapeJs(
            $helper->__('"%title%" task has completed with errors. <a href="%url%">View log</a> for details.')
        );

        $lockedObjNoticeMessage = $helper->escapeJs($helper->__('Some Amazon request(s) are being processed now.'));
        $sendingDataToAmazonMessage = $helper->escapeJs($helper->__('Sending %execute% product(s) data on Amazon.'));
        ...
        $productsWereNotMovedMessage = $helper->escapeJs(
            $helper->__('Product(s) was not moved. <a href="%url%">View log</a> for details.')
        );
        $someProductsWereNotMovedMessage = $helper->escapeJs(
            $helper->__('Some product(s) was not moved. <a href="%url%">View log</a> for details.')
        );
...
JAVASCRIPT;
    ...
    public function getEditTemplateDropDownHtml()
    {
        ...
        $descriptionUrl = $this->getUrl('*/adminhtml_amazon_template_description/edit', array(
...
<ul style="display: none;">
    ...
    <li href="{$descriptionUrl}" target="_blank">{$descriptionTemplate}</li>
