...
class Ess_M2ePro_Adminhtml_Amazon_AccountController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            // ->__('The Amazon access obtaining is currently unavailable.<br />Reason: %reason%')
            $error = 'The Amazon access obtaining is currently unavailable.<br />Reason: %reason%';
            $error = Mage::helper('M2ePro')->__($error);
            $error = str_replace('%reason%', $exception->getMessage(), $error);
    ...
    public function checkAuthAction()
    {
        ...
        if ($merchantId && $marketplaceId) {
            ...
            $marketplaceNativeId = Mage::getModel('M2ePro/Marketplace')->load($marketplaceId)->getNativeId();
