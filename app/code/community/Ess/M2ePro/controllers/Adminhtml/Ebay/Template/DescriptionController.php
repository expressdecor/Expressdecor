...
class Ess_M2ePro_Adminhtml_Ebay_Template_DescriptionController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    ...
    public function saveAction()
    {
        ...
        if ($data['watermark_mode'] == Ess_M2ePro_Model_Ebay_Template_Description::WATERMARK_MODE_YES) {
            if (!is_null($id)) {
                $tempModel = Mage::helper('M2ePro/Component_Ebay')->getModel('Template_Description');
                $watermarkSettings = $tempModel->load($id)->getSettings('watermark_settings');
            } else {
            ...
            if (!$id) {
                ...
                if (!$product->getId()) {
                    ...
                    $errorTxt = str_replace(
                        array('%id%','%attr%'),
                        array($id,Mage::helper('M2ePro')->__($attributeSetsTitles)),
                        Mage::helper('M2ePro')->__('Product #%id% does not belong to "%attr%" attribute set(s).')
                    );
                } else {
