...
class Ess_M2ePro_Adminhtml_Ebay_Template_GeneralController extends Ess_M2ePro_Controller_Adminhtml_MainController
{
    ...
    public function editAction()
    {
        ...
        if ($collection->addFieldToFilter('status',1)->getSize() == 0) {
            ...
            $error  = Mage::helper('M2ePro')->__('Please select and update eBay <a href="%url%" target="_blank">');
            $error .= Mage::helper('M2ePro')->__('marketplaces</a> before adding new General Templates.');
            $this->_getSession()->addError(str_replace('%url%',$url,$error));
        ...
        if (Mage::getModel('M2ePro/Ebay_Account')->getCollection()->getSize() == 0) {
            ...
            $errorMessage  = Mage::helper('M2ePro')->__('Please add eBay <a href="%url%" target="_blank">accounts</a>');
            $errorMessage .= Mage::helper('M2ePro')->__(' before adding new General Templates.');
            $this->_getSession()->addError(str_replace('%url%',$url,$errorMessage));
            return $this->_redirect('*/adminhtml_template_general/index');
        }
        //-------------------------
    ...
    public function getChildCategoriesAction()
    {
        ...
        $data = Mage::getModel('M2ePro/Ebay_Marketplace')->loadInstance($marketplaceId)
    ...
    public function getCategoriesTreeByCategoryIdAction()
    {
        ...
        for ($i = 1; $i < 8; $i++) {
            ...
            $category = Mage::getModel('M2ePro/Ebay_Marketplace')->loadInstance($marketplaceId)
                ->getCategory($categoryId);

            if (!$category) {
                $data['error']  = Mage::helper('M2ePro')->__('Category with ID: %id% is not found. ');
    ...
    public function saveAction()
    {
        ...
        //file_put_contents(MAGENTO_ROOT.DS.'tmp', print_r($internationalTrade, true));
