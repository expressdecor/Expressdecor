...
//++++ QT Pro: End Changed Code
    ...
    if ( (isset($order->products[$i]['attributes'])) && (sizeof($order->products[$i]['attributes']) > 0) ) {
      foreach($order->products[$i]['attributes'] as $option => $value) {
        if(preg_match("/bundle_id/i", $option)) {
            foreach($value as $bundle_id => $bundle_option_id) {
                if(is_array($bundle_option_id)) {
                    foreach($bundle_option_id as $bundle_option_id_key => $bundle_option_id_qty) {
                        $b_info = getBundleProduct($order->products[$i]['id'], $bundle_id, $bundle_option_id_key);
                        $bundle_option_id_qty = checkBundleOptionQty($bundle_id, $bundle_option_id_key, $bundle_option_id_qty);
                        echo '<br><nobr><small>&nbsp; - ' . $b_info['bundle_name'] . ': '. $bundle_option_id_qty .' x ' . $b_info['products_name'] . '</small></nobr>';
                    }
                }
            }
        }
        ...
        {
            echo '<br><nobr><small>&nbsp;<i> - ' . $value['option'] . ': ' . $value['value'] . '</i></small></nobr>';
        }
      }
    }
