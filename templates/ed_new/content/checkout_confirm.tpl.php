...
<table border="0" width="100%" cellspacing="0" cellpadding="4">
	<tr>
		<td>
			<table width="100%">
				<tr>
					<td><span style="color:red; font-weight:bold; font-size: 14px;">Please make sure that the address that you have put in the "Your billing address" field is the address that appears on the monthly credit card statement of a credit card is used (i.e. the address where the credit card bills are sent each month).</span>&nbsp;&nbsp;<a href="javascript:{};" onclick="javascript:{window.open('./disclamer_cc.htm','test','height=450,width=800,top=100,left=100');false}">Read more...</a></td>
					<td align="right">
<?php
						...
						//// Added foe discount
							if (isset($_SERVER['PHP_AUTH_USER']) && $_SERVER['PHP_AUTH_USER'] == 'exdecor') {
								...
								echo tep_draw_hidden_field('discount2',$discount);
							}
						//// EO Added for discount
						  if (is_array($payment_modules->modules)) {
						    echo $payment_modules->process_button();
						  }
						
						  echo tep_image_submit('button_confirm_order.gif', IMAGE_BUTTON_CONFIRM_ORDER, ' onClick="visi(\'checkoutBtn\');" id="checkoutBtn"') . '
						  <br><span style="visibility: hidden;" ID="altText">Please wait...</span></form>' . "\n";
?>
					</td>
				</tr>
			</table>
...
<?php
  ...
  if (isset($_SERVER['PHP_AUTH_USER']) && $_SERVER['PHP_AUTH_USER'] == 'exdecor') {
	if (isset($GLOBALS['cart']->discount1)) {
		$discount1 = $GLOBALS['cart']->discount1;
	} else {
		$discount1 = 0;
	}
	if (isset($GLOBALS['cart']->discount2)) {
		$discount2 = $GLOBALS['cart']->discount2;
	} else {
		$discount2 = 0.00;
	}
?>
              <tr>
                <td class="main"><b>Discount $:</b></td>
                <td class="main">$<?php echo $discount2; ?></td>
              </tr>
              <tr>
                <td class="main"><b>Discount %:</b></td>
                <td class="main"><?php echo $discount1;?>%</td>
              </tr>
<?php
  }
?>
...
//// EO Added for discount
  ...
  echo tep_image_submit('button_confirm_order.gif', IMAGE_BUTTON_CONFIRM_ORDER, ' onClick="visi(\'checkoutBtn2\');" id="checkoutBtn2"') . '
  <br><span style="visibility: hidden;" ID="altText2">Please wait...</span></form>' . "\n";
?>
            </td>
          </tr>
        </table></td>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td align="center" width="25%" class="checkoutBarFrom"><?php echo '<a href="' . tep_href_link('checkout.php', '', 'SSL') . '" class="checkoutBarFrom">Checkout Details</a>'; ?></td>
