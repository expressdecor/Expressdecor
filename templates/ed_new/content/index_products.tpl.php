...
?>
	  ...
	  <?php if ($category['categories_text1']!='' && (!isset($HTTP_GET_VARS['page']) || isset($HTTP_GET_VARS['page']) && $HTTP_GET_VARS['page'] == 1) && (empty($_GET['sort']) || $_GET['sort'] == '2a' )) { ?>
	  	<div class="homepage_text"><?php echo $category['categories_text1']; ?></div>
	  <?php } ?>
		</div>
		
		<?php if ($category['categories_text2']!='' && (!isset($HTTP_GET_VARS['page']) || isset($HTTP_GET_VARS['page']) && $HTTP_GET_VARS['page'] == 1) && (empty($_GET['sort']) || $_GET['sort'] == '2a' )) { ?>
		<div class="border homepage_text" style="margin-top:10px;margin-bottom:10px;"><?php echo $category['categories_text2']; ?></div>
		<?php } ?>
		<?php include(DIR_WS_MODULES . FILENAME_PRODUCT_LISTING_COL); ?>
		
		<?php if ($category['categories_text3']!='' && (!isset($HTTP_GET_VARS['page']) || isset($HTTP_GET_VARS['page']) && $HTTP_GET_VARS['page'] == 1)  && (empty($_GET['sort']) || $_GET['sort'] == '2a' )) { ?>
